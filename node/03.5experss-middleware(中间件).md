...
```javascript
app.use('/a',function(req,res,next){//必须是/a开头的请求去路径
    console.log('Time',Date.now());
    next();
});
```
...
app.use(function(err,req,res,next){
    res.status(500).json({
        err_code:500,
        message:err.message//错误信息是err的错误信息
    });
});
```